...
export class ObservationEntityComponent {
  ...
  ngOnInit() {
    ...
    console.log('sa', this.apiService.profileData)
    console.log('two', this.apiService.profileData[this.entityToAdd])
    ...
    this.apiService.post(urlConfig.observation.getSelectedEntities + this.solutionId, this.apiService.profileData)
      ...
      .subscribe((res: any) => {
        ...
        if (res.result) {
          ...
          this.observationId = this.selectedEntities?._id
        } else {
  ...
  handleSearchInput(event: any) {
    ...
    item?.metaInformation?.name.toLowerCase().includes(searchValue)
